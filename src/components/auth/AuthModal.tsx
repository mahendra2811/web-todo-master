    ...
    >
      <div className="w-full max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-xl bg-white p-5 sm:p-6 shadow-lg sm:max-w-md sm:mx-4">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={close} className="p-1 -mr-1 text-gray-400 hover:text-gray-600 transition-colors">
...
function LoginView({ onSwitch, onClose }: { onSwitch: (v: 'signup' | 'forgot') => void; onClose: () => void }) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <button type="button" onClick={() => onSwitch('forgot')}
          className="mt-1 text-xs text-indigo-600 hover:text-indigo-700">Forgot password?</button>
      </div>
      ...
      <button type="button" onClick={() => { const supabase = createClient(); supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/dashboard` } }); }}
      ...
      <p className="text-center text-sm text-gray-600">
        ...
        <button type="button" onClick={() => onSwitch('signup')} className="text-indigo-600 hover:text-indigo-700 font-medium">Sign up</button>
      </p>
    </form>
  );
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-center text-sm text-gray-600">
        ...
        <button type="button" onClick={() => onSwitch('login')} className="text-indigo-600 hover:text-indigo-700 font-medium">Sign in</button>
      </p>
    </form>
  );
}
  ...
  if (sent) {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-sm text-gray-600 mb-4">We sent a password reset link to <strong>{email}</strong></p>
        <button type="button" onClick={() => onSwitch('login')}
          className="text-sm text-indigo-600 hover:text-indigo-700 font-medium">Back to Sign In</button>
      </div>
    );
  }
