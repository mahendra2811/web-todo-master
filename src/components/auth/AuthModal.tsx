    ...
    >
      <div className="w-full max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl bg-white p-6 sm:p-8 shadow-2xl sm:max-w-md sm:mx-4">
        <div className="flex items-center justify-between mb-1">
          <div />
          <button onClick={close} className="p-1.5 -mr-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all">
  ...
  return (
    <div>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <button type="button" onClick={() => onSwitch('forgot')}
              className="text-xs font-medium text-indigo-600 hover:text-indigo-700 transition-colors">Forgot password?</button>
          </div>
      ...
      <p className="text-center text-sm text-gray-500 mt-5">
        ...
        <button type="button" onClick={() => onSwitch('signup')} className="font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">Create one free</button>
      </p>
    </div>
  );
}
...
function SignupView({ onSwitch }: { onSwitch: (v: 'login') => void }) {
  ...
  const strength = password.length === 0 ? 0 : password.length < 6 ? 1 : password.length < 8 ? 2 : /(?=.*[A-Z])(?=.*\d)/.test(password) ? 4 : 3;
  ...
  return (
    <div>
      ...
      <p className="text-center text-sm text-gray-500 mt-5">
        ...
        <button type="button" onClick={() => onSwitch('login')} className="font-semibold text-indigo-600 hover:text-indigo-700 transition-colors">Sign in</button>
      </p>
    </div>
  );
}
  ...
  if (sent) {
    return (
      <div className="text-center py-2">
        ...
        <p className="text-sm text-gray-500">We sent a reset link to <strong className="text-gray-700">{email}</strong></p>
